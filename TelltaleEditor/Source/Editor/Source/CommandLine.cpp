...
namespace CommandLine
{
    ...
    static I32 Executor_LoadAll(const std::vector<TaskArgument>& args)
    {
        ...
        Bool bNormalise = GetBoolArgumentOrDefault(args, "-normalise", true);
        std::set<String> nonMeta{}, failedOpen{}, failed{}, failedNormalise{}, failedNoCommonClass{}, okFiles{};
        U32 numOk = 0;
        std::vector<String> infiles = GetInputFiles(inf);
        if(!infiles.size())
            return 1;
        ...
        Ptr<ResourceRegistry> reg = editor->CreateResourceRegistry(false);
        {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if(instance)
                        {
                            ...
                            if(bNormalise)
                            {
                                if(CompareCaseInsensitive(ext, "prop"))
                                {
                                    ok = true;
                                }
                                else
                                {
                                    ok = false;
                                    const auto& info = CommonClassInfo::GetCommonClassInfoByExtension(ext);
                                    Ptr<Handleable> pResource = info.ClassAllocator ? info.ClassAllocator(reg) : nullptr;
                                    if (pResource)
                                    {
            ...
            }
            {
                ...
                log << "\n** The following files passed:\n\n";
                for(const auto& f: okFiles)
                    log << "** " << f << "\n";
        ...
        Bool ParseUserArgument(TaskArgument& arg, std::vector<String>& argsStack)
        {
            ...
            if(arg.Type == ArgumentType::BOOL)
            {
                ...
                if(boolVal[0] == 't' || boolVal[0] == 'T' ||  boolVal[0] == 'y' || boolVal[0] == 'Y')
                    arg.BoolValue = true;
                else if(boolVal[0] == 'f' || boolVal[0] == 'F' ||  boolVal[0] == 'n' || boolVal[0] == 'N')
                    arg.BoolValue = false;
                else
                {
